...
class WebcamVideoStream:
    def __init__(self, src=0):
        ...
        self.lower_bound_Dodgebot = np.array([100, 100, 100])
        self.upper_bound_Dodgebot = np.array([100, 255, 255])
        self.mapx = np.load("mapx.npy")
        self.mapy = np.load("mapy.npy")
        self.punch_pixel_cords = 0
        self.stopped = False
    ...
    def start(self):
        ...
        threading.Thread(target=self.processed, args=()).start()
        time.sleep(1)
        threading.Thread(target=self.gloves_marked_pixels, args=()).start()
        # threading.Thread(target=self.processed, args=()).start()
    ...
    def update(self):
        ...
        while True:
            ...
            if self.stopped:
                ...
                # (self.grabbed, self.frame) = self.stream.read()
                (self.grabbed,
                 self.frame) = self.stream.read()  # print("grabbed", self.grabbed)  # self.frame = frame_resize(
                # cv.remap(temp_frame, self.mapx, self.mapy, cv.INTER_LINEAR))

        # otherwise, read the next frame from the stream  # (self.grabbed, self.frame) = self.stream.read()

    def processed(self):
        while True:
            if self.grabbed == True and self.processed_frame == False:
                self.frame = frame_resize(cv.remap(self.frame, self.mapx, self.mapy, cv.INTER_LINEAR))
                self.grabbed = False
    ...
    def read(self, temp_frame):
        if self.processed_frame == True:
            temp_frame[0] = self.frame
    ...
    # 	if self.grabbed == True:
    # 		self.grabbed = False
    # 		frame[0] = frame_resize(cv.remap(self.frame, self.mapx, self.mapy, cv.INTER_LINEAR))
    # 	else:
    # 		return -1


    # temp = frame_resize(cv.remap(self.frame, self.mapx, self.mapy, cv.INTER_LINEAR))
    # temp = frame_resize(temp)
    # temp = self.frame
    # self.grabbed = False

    # return temp
    def gloves_marked_pixels(self):
        i = 0
        while True:
            if self.processed_frame == True:
                # self.grabbed = False
                ...
                self.punch_pixel_cords = np.column_stack(np.where(color_mask_gloves > 0))
                self.processed_frame = False
                i += 1
            else:
                self.punch_pixel_cords = -1

            if self.stopped == True:
                break
        print("stopped looking for glove pixels ", i)


    def return_glove_pixels(self):
        # if type(self.punch_pixel_cords) != int:
        return self.punch_pixel_cords

    # else:
    # return


    def marked_pixels(self):
        # while True:
        # 	if self.stopped:
        # 		break
        ...
        color_mask_gloves = cv.inRange(hsv_frame, self.lower_bound_gloves, self.upper_bound_gloves)
        color_mask_Dodgebot = cv.inRange(hsv_frame, self.lower_bound_Dodgebot, self.upper_bound_Dodgebot)

        # np.column_stack(np.where(color_mask_Dodgebot > 0))
        return np.column_stack(np.where(color_mask_gloves > 0)), np.column_stack(
                np.where(color_mask_Dodgebot > 0)), frame
...
# def Dodgebot_center(dodgebot_pixels, dodgebot_cordinates, dodgebot_cordinates_polar, real_center_dist, cam_height,
# Dogebot_height, Dodgebot_Radius, camera_orientation):
# 	while True:

# 		if real_center_dist != past_center_dist:


# 			past_center_dist = real_center_dist.copy()

def Punch_centers(punch_pixels,
                  pixel_center,
                  real_center_dist,
                  cam_height,
                  user_height,
                  ...
                  update):
    keep_eval = True
    while keep_eval == True:
        # marked_pixel_coords, _ , frame_grab = cam.marked_pixels()
        # frame = frame_grab.copy()
        # if len(punch_pixels)
        # print(punch_pixels[0])
        # print(update)
        # if update[0] == True:
        if len(punch_pixels[0]) > 1:
            marked_coords = pixel_2_cords(punch_pixels[0], pixel_center, real_center_dist, cam_height, user_height,
                                          camera_orientation)
            # run_time_dict["Converting Cords"].append(time.time()-st)
            cord_list, cord_centers = kmeans_gpu(marked_coords, 2, cord_centers)
            # punch_pixels[0] = []
            centroid1[0] = cord_centers[0].numpy()
            # centroid1[0][1] = cord_centers[0][1]

            centroid2[0] = cord_centers[1].numpy()
        # print("thread centroids", centroid1, centroid2)
        # centroid2[0][1] = cord_centers[1][1]
        else:
        ...
        # break
        # print(cord_centers)
        if len(cord_centers[0]) == 0:
            keep_eval = False
            break
    # time.sleep(.001)
    print("Stop Center Looking")
...
robot_loc_pixel = cords_2_pixel(robot_loc, pixel_center, real_center_dist, cam_height, user_height,
                                inv_camera_orientation)
max_dodge_dist = .1
avoidance_dist = .15
...
dodgebot_pixel_coords = [[]]
punch_pixel_coords = [[]]
centroid1_thread = [np.array([0, 0])]
centroid2_thread = [np.array([0, 0])]
update = [False]
...
# distortion_coeff_640x360 = np.array([[-3.52228685e-01, 1.55541160e-01,4.99708023e-05,-4.91499027e-05,
# -3.67726346e-02]])
...
i = 0
dodge_time = 0
# run_time_dict = {"Un-Distortion":[], "Resize":[], "Color Detection":[],"Marking Pixels":[], "Converting Cords":[],
# "Kmeans":[],"Buffer":[],"Main Punch":[],"Slope":[], "Punch Vector Calc":[], "Avoidance":[], "Total End":[],
# "Output End":[]}
...
while True:
    # cam.read(temp_frame)
    # test_frame = temp_frame[0]
    # glove_pixels = cam.return_glove_pixels()
    # print(type(glove_pixels))
    # if type(glove_pixels) != int:
    # i += 1
    # print(i)
    # cv.imshow('Original Frame', test_frame)
    i += 1
    if cv.waitKey(1) == ord('q') or i == 5000:
        ...
        break
print((
                  time.time() - st1))  # punch_center_thread = threading.Thread(target=Punch_centers,
# args=(punch_pixel_coords, pixel_center, real_center_dist, cam_height, user_height, camera_orientation,
# cord_centers, centroid1_thread, centroid2_thread,update))
# punch_center_thread.start()
...
# 	punch_pixel_coords_temp, dodgebot_pixel_coords_temp, frame = cam.marked_pixels()
# 	punch_pixel_coords[0] = punch_pixel_coords_temp
# 	update[0] = True
# 	dodgebot_pixel_coords[0] = dodgebot_pixel_coords_temp.copy()
# 	# print(punch_pixel_coords[0])
# 	# print(punch_pixel_coords_temp.shape)
# 	# print(len(punch_pixel_coords_temp.shape))
# 	# print(len(punch_pixel_coords_temp))


# 	# run_time_dict["Marking Pixels"].append(time.time()-st)
# 	# marked_coords = pixel_2_cords(punch_pixel_coords_temp.copy(), pixel_center, real_center_dist, cam_height,
...
# 	if len(punch_pixel_coords_temp) > 1:
# 		centroid1 = centroid1_thread[0]
# 		centroid2 = centroid2_thread[0]
# 		# print("Centroids", centroid1, centroid2)
# 		# print(centroid2_thread[0])
...
# 			main_centroid = cords_2_pixel(centroid1, pixel_center, real_center_dist, cam_height, user_height,
# 			inv_camera_orientation)
# 			minor_centroid = cords_2_pixel(centroid2, pixel_center, real_center_dist, cam_height, user_height,
# 			inv_camera_orientation)
# 		else:
...
# 			main_centroid = cords_2_pixel(centroid2, pixel_center, real_center_dist, cam_height, user_height,
# 			inv_camera_orientation)
# 			minor_centroid = cords_2_pixel(centroid1, pixel_center, real_center_dist, cam_height, user_height,
# 			inv_camera_orientation)
...
# 				# if (np.abs(robot_loc[0]) >= edge_percentage*real_center_dist_user) or (np.abs(robot_loc[1]) >=
# 				edge_percentage*real_center_dist_user):
# 					print("Edge Dodge")
...
# 					# if np.linalg.norm(perpendicular_v1 - robot_2_body_v_hat) < np.linalg.norm(perpendicular_v2 -
# 					robot_2_body_v_hat):

# 					avoidance_vector = np.abs(avoidance_dist - dist_from_punch_traj) * hat_avoidance_vector + (2 *
...
# 					hat_avoidance_vector = np.array([vx, vy])
# 					avoidance_vector = np.abs(avoidance_dist - dist_from_punch_traj) * hat_avoidance_vector + (1.1 *
# 					avoidance_dist) * hat_avoidance_vector
...
# 			prev_robot_loc_pixel =  cords_2_pixel(prev_robot_loc, pixel_center, real_center_dist, cam_height,
# 			user_height, inv_camera_orientation)
...
# 	else:
# 		cord_centers[0] = torch.tensor([[float('nan'), -.5], [.5, .5]])
# 		robot_loc = np.array([0, 0])
# 		robot_loc_pixel = cords_2_pixel(robot_loc, pixel_center, real_center_dist, cam_height, user_height,
# 		inv_camera_orientation)

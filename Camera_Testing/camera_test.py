...
class WebcamVideoStream:
	...
	def update(self):
		...
		while True:
			...
			if self.stopped:
				...
				# self.frame = frame_resize(cv.remap(temp_frame, self.mapx, self.mapy, cv.INTER_LINEAR))
	...
	def marked_pixels(self):
		...
		return np.column_stack(np.where(color_mask_gloves > 0)), np.column_stack(np.where(color_mask_Dodgebot > 0)), frame
	
	def stop(self):
		# indicate that the thread should be stopped
		self.stopped = True

# def Dodgebot_center(dodgebot_pixels, dodgebot_cordinates, dodgebot_cordinates_polar, real_center_dist, cam_height, Dogebot_height, Dodgebot_Radius, camera_orientation):
# 	while True:
...
def Punch_centers(punch_pixels, pixel_center, real_center_dist, cam_height, user_height, camera_orientation, cord_centers, centroid1, centroid2, update):
	keep_eval = True
	while keep_eval == True:
		...
		if len(punch_pixels[0]) > 1:
			marked_coords = pixel_2_cords(punch_pixels[0], pixel_center, real_center_dist, cam_height, user_height, camera_orientation)
...
while True:
	...
	# cord_centers[0] = torch.tensor([[float('nan'), float('nan')], [float('nan'), float('nan')]])
		...
		break
print((time.time() - st1))
# punch_center_thread = threading.Thread(target=Punch_centers, args=(punch_pixel_coords, pixel_center, real_center_dist, cam_height, user_height, camera_orientation, cord_centers, centroid1_thread, centroid2_thread,update))
...
# 	# marked_coords = pixel_2_cords(punch_pixel_coords_temp.copy(), pixel_center, real_center_dist, cam_height, user_height, camera_orientation)
# 	# run_time_dict["Converting Cords"].append(time.time()-st)
# 	# cord_list, cord_centers = kmeans_gpu(marked_coords, 2, cord_centers)

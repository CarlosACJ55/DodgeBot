...
def kmeans_gpu(identified_punch_cords, num_gloves, prev_cluster_centers, num_punches = 0):
...
while cap.isOpened():
    ...
    if len(cord_list) == 2:
        ...
        if len(centroid_buffer1) >= buffer_size and len(centroid_buffer1) >= buffer_size:
            #takes out the last vaules in the buffer
        ...
        if punch1_dist > punch2_dist:
            ...
            centroid = centroid1
            centroid_ = centroid2
            # centroid = cords_2_pixel(centroid1, pixel_center, real_center_dist, cam_height, user_height, camera_orentation)
            # centroid_ = cords_2_pixel(centroid2, pixel_center, real_center_dist, cam_height, user_height, camera_orentation)
            ...
            centroid = centroid2
            centroid_ = centroid1
            # centroid = cords_2_pixel(centroid2, pixel_center, real_center_dist, cam_height, user_height, camera_orentation)
            # centroid_ = cords_2_pixel(centroid1, pixel_center, real_center_dist, cam_height, user_height, camera_orentation)
        ...
        # punch_trajectory_line_slope = (main_punch[len(main_punch)-1][1] - main_punch[0][1]) / (main_punch[len(main_punch)-1][0] - main_punch[0][0])
        ...
        dist_from_punch_traj = torch.norm(torch.tensor([previous_robot_loc[0], punch_y_check], dtype=torch.float16).to(device) - previous_robot_loc)
        ...
        if (abs(previous_robot_loc[0]) >= .5) or (abs(previous_robot_loc[1]) >= .5) or torch.isnan(previous_robot_loc).any():

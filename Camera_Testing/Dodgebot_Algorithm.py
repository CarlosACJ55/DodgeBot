...
class WebcamVideoStream:
    def __init__(self, src=0):
        ...
        (self.grabbed, self.f) = self.stream.read()
    ...
    def read(self):
        ...
        temp = cv.remap(self.f, self.mapx, self.mapy, cv.INTER_LINEAR)
...
while True:
    ...
    marked_coords = pixel_2_cords(marked_pixel_coords.copy(), pixel_center, real_center_dist, cam_height, user_height,
                                  camera_orientation)
    ...
    if len(cord_list) == 2:
            ...
            if dist_from_punch_traj <= avoidance_dist:
                ...
                if np.linalg.norm(robot_loc) >= edge_percentage * Radius:
                    ...
                    avoidance_vector = np.abs(avoidance_dist - dist_from_punch_traj) * hat_avoidance_vector + (
                            1.5 * avoidance_dist) * hat_avoidance_vector  # else:  # avoidance_vector = np.abs(  #  #
                    # avoidance_dist - dist_from_punch_traj) * hat_avoidance_vector + ( * avoidance_dist) *   #  #  #
                    # hat_avoidance_vector
        ...
        if list(prev_robot_loc) != [-10, -10]:
            ...
            cv.circle(frame, tuple(robot_loc_pixel[::-1]), 25, (255, 255, 255),
                      -1)  # print("Pixel Robot: ", robot_loc_pixel)
        else:
